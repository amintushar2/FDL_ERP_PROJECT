@extends('layouts.app') @section('title', 'Department Setup') @include('hrm.setup.partials.styles')
@section('content')
    ...
    <div class="setup-page-wrap">
        ...
        <div class="setup-main">
            <div class="setup-page-header">
                ...
                <p class="bc">HRM <span>›</span> Setup <span>›</span> Department</p>
            </div>
                ...
                <div class="setup-card">
                    <div class="setup-card-hd">
                        <div class="card-hd-left">
                            <div class="card-dot"></div><span class="card-ttl">Department Setup</span>
                        </div>
                        ...
                        <table class="setup-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:120px">DEPT NO</th>
                                    <th>DEPT NAME</th>
                            ...
                            <tbody>
                                @forelse($records as $i=>$row)
                                    <tr data-id="{{ $row->dept_no }}">
                                        <td class="mono">{{ str_pad($records->firstItem() + $i, 2, '0', STR_PAD_LEFT) }}
                                        </td>
                                        <td><span class="mono">{{ $row->dept_no }}</span></td>
                                        <td style="font-weight:700">{{ $row->dept_name }}</td>
                                        <td>{{ $row->in_short }}</td>
                                        <td>{{ $row->company_name ?? $row->company_id }}</td>
                                        <td>
                                            <button class="btn-row-edit" onclick="openEditModal('{{ $row->dept_no }}')"><svg
                                                    width="11" height="11" viewBox="0 0 16 16" fill="none"
                                                    stroke="currentColor" stroke-width="2">
                                                    <path d="M11 2l3 3-9 9H2v-3z" />
                                                </svg>Edit</button>
                                            <button class="btn-row-del"
                                                onclick="askDelete('{{ $row->dept_no }}','{{ addslashes($row->dept_name) }}')"><svg
                                                    width="11" height="11" viewBox="0 0 16 16" fill="none"
                                                    stroke="currentColor" stroke-width="2">
        ...
        <div class="modal-dialog modal-dialog-centered" style="max-width:520px">
            <div class="modal-content">
                <div class="modal-header modal-header-navy"><span class="modal-title" id="modalTitle"><svg
                            ...
                            <line x1="8" y1="2" x2="8" y2="14" />
                            <line x1="2" y1="8" x2="14" y2="8" />
                        </svg> Add Department</span><button type="button" class="btn-close"
                        data-bs-dismiss="modal"></button></div>
                <div class="modal-body p-4"><input type="hidden" id="recordId">
                    <div class="row g-3">
                        <div class="col-6"><label class="f-lbl">Dept No <span
                                    class="text-danger">*</span></label><input type="text" id="fDeptNo"
                                class="f-inp" placeholder="e.g. D001"
                                style="font-family:'DM Mono',monospace;text-transform:uppercase">
                            <div class="err-msg" id="fDeptNo_err"></div>
                        </div>
                        <div class="col-6"><label class="f-lbl">Short Name</label><input type="text" id="fShort"
                                class="f-inp" placeholder="e.g. HR"></div>
                        <div class="col-12"><label class="f-lbl">Department Name <span
                                    class="text-danger">*</span></label><input type="text" id="fName"
                                class="f-inp" placeholder="Full department name">
                        ...
                        <div class="col-12"><label class="f-lbl">Company</label><select id="fCompany" class="f-sel">
...
@push('scripts')
    <script>
        const BASE_URL = '{{ url('setup/department') }}';
        const crudModal = new bootstrap.Modal(document.getElementById('crudModal'));

        function openAddModal() {
            document.getElementById('recordId').value = '';
            document.getElementById('fDeptNo').value = '';
            document.getElementById('fName').value = '';
            document.getElementById('fShort').value = '';
            document.getElementById('fBengali').value = '';
            document.getElementById('fCompany').value = '';
            document.getElementById('fDeptNo').readOnly = false;
            document.getElementById('modalTitle').innerHTML =
                `<svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2"><line x1="8" y1="2" x2="8" y2="14"/><line x1="2" y1="8" x2="14" y2="8"/></svg> Add Department`;
            clearModalErrors('crudModal');
            crudModal.show();
            setTimeout(() => document.getElementById('fDeptNo').focus(), 150);
        }
        async function openEditModal(id) {
            try {
                ...
                const r = data.record;
                document.getElementById('recordId').value = r.dept_no;
                document.getElementById('fDeptNo').value = r.dept_no;
                document.getElementById('fDeptNo').readOnly = true;
                document.getElementById('fName').value = r.dept_name || '';
                ...
                document.getElementById('modalTitle').innerHTML =
                    `<svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M11 2l3 3-9 9H2v-3z"/></svg> Edit Department`;
                clearModalErrors('crudModal');
                crudModal.show();
            } catch (e) {
        ...
        async function saveRecord() {
            clearModalErrors('crudModal');
            let valid = true;
            if (!validateModalField('fDeptNo', 'Dept No is required.')) valid = false;
            ...
            try {
                const {
                    ...
                    dept_no: document.getElementById('fDeptNo').value.trim().toUpperCase(),
                    dept_name: document.getElementById('fName').value.trim(),
