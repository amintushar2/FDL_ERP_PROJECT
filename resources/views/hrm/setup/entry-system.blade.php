@extends('layouts.app') @section('title', 'Entry System Setup') @include('hrm.setup.partials.styles')
@section('content')
    ...
    <div class="setup-page-wrap">
        ...
        <div class="setup-main">
            <div class="setup-page-header">
                <h4>Entry System Setup</h4>
                <p class="bc">HRM <span>›</span> Setup <span>›</span> Entry System</p>
            </div>
            ...
            <div class="setup-body">
                <div class="sec-hd">
                    <div class="sec-title"><svg width="15" height="15" viewBox="0 0 16 16" fill="none"
                        ...
                        </svg>Entry System</div>
                    <div class="sec-line"></div>
                </div>
                <div class="setup-card">
                    <div class="setup-card-hd">
                        <div class="card-hd-left">
                            <div class="card-dot"></div><span class="card-ttl">Entry System Setup</span>
                        </div>
                        <div class="d-flex align-items-center gap-2"><input class="card-search" id="cardSearch"
                                placeholder="Search…"><button class="btn-add-record" onclick="openAddModal()"><svg
                                    ...
                                    <line x1="8" y1="2" x2="8" y2="14" />
                                    <line x1="2" y1="8" x2="14" y2="8" />
                                </svg>+ Add Entry System</button></div>
                    </div>
                            ...
                            <tbody>
                                @forelse($records as $i=>$row)
                                    <tr data-id="{{ $row->id }}">
                                        ...
                                        <td style="font-weight:700">{{ $row->entry_name }}</td>
                                        <td><span class="mono">{{ $row->entry_code }}</span></td>
                                        <td>
                                        ...
                                        <td><button class="btn-row-edit" onclick="openEditModal({{ $row->id }})"><svg
                                                    width="11" height="11" viewBox="0 0 16 16" fill="none"
                                                    stroke="currentColor" stroke-width="2">
                                                    <path d="M11 2l3 3-9 9H2v-3z" />
                                                </svg>Edit</button><button class="btn-row-del"
                                                onclick="askDelete({{ $row->id }},'{{ addslashes($row->entry_name) }}')"><svg
                                                    width="11" height="11" viewBox="0 0 16 16" fill="none"
                                                    stroke="currentColor" stroke-width="2">
        ...
        <div class="modal-dialog modal-dialog-centered" style="max-width:460px">
            <div class="modal-content">
                <div class="modal-header modal-header-navy"><span class="modal-title" id="modalTitle"><svg
                            ...
                            <line x1="8" y1="2" x2="8" y2="14" />
                            <line x1="2" y1="8" x2="14" y2="8" />
                        </svg> Add Entry System</span><button type="button" class="btn-close"
                        data-bs-dismiss="modal"></button></div>
                <div class="modal-body p-4"><input type="hidden" id="recordId">
                    <div class="mb-3"><label class="f-lbl">Entry System Name <span
                    ...
                    <div class="mb-3"><label class="f-lbl">Code <span class="text-danger">*</span></label><input
                            type="text" id="fCode" class="f-inp" placeholder="e.g. ENT01"
...
@push('scripts')
    <script>
        const BASE_URL = '{{ url('setup/entry-system') }}';
        ...
        function openAddModal() {
            ...
            document.getElementById('modalTitle').innerHTML =
                `<svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2"><line x1="8" y1="2" x2="8" y2="14"/><line x1="2" y1="8" x2="14" y2="8"/></svg> Add Entry System`;
        ...
        async function openEditModal(id) {
            try {
                ...
                document.getElementById('fName').value = r.entry_name || '';
                document.getElementById('fCode').value = r.entry_code || '';
                document.getElementById('fStatus').value = r.is_active || 'Y';
                document.getElementById('modalTitle').innerHTML =
                    `<svg width="13" height="13" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M11 2l3 3-9 9H2v-3z"/></svg> Edit Entry System`;
                clearModalErrors('crudModal');
                crudModal.show();
            } catch (e) {
        ...
        async function saveRecord() {
            ...
            try {
                const {
                    ...
                    entry_name: document.getElementById('fName').value.trim(),
                    entry_code: document.getElementById('fCode').value.trim().toUpperCase(),
